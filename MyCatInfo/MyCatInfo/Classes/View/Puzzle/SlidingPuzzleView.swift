...
class SlidingPuzzleView: UIView {
    ...
    private func randomSwap() {
        guard let emptyPieceViewIndex = self.imagePiecesArray.firstIndex(of: self.emptyPieceView) else {
    ...
    @objc private func tapGestureAction(gesture: UITapGestureRecognizer) {
        ...
        guard let tapViewIndex = self.imagePiecesArray.firstIndex(of: view) else {
