...
class SCMoreViewController: UIViewController {
    private let titleInfoArray = [
        ...
        ["title": "Remove all favourite breeds: ", "iconName": "btn_remove", "content":"Select and confirm to delete all your favourite records."],
...
extension SCMoreViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 3:
            // create the alert
            let alert = UIAlertController(title: "Notice", message: "Would you like to continue removing all your favourite breeds?", preferredStyle: UIAlertController.Style.alert)
            
            // add the actions (buttons)
            alert.addAction(UIAlertAction(title: "Confirm", style: UIAlertAction.Style.default, handler: { (_) in
                CoreDataManager.shared.removeAllFavouriteBreeds()
            }))
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        let viewWidth = UIScreen.screenWidth() - 3 * margin - iconWidth
        var height = margin
        height += titleLabelHeight
        height += margin
        let content = titleInfoArray[indexPath.row]["content"] ?? ""
        height += content.heightWithConstrainedWidth(width: viewWidth, font: UIFont.systemFont(ofSize: 15))
        height += margin
        return height
    }
}
